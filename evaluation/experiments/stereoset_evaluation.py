"""
This script aggregates Language Modelling Scores, Stereotype Scores and idealised CAT scores for previously generated
predictions on StereoSet.
...
Functions needed to evaluate other de-biasing techniques tested by Meade et al. (2022) are omitted for simplicity.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="dev_en.json",
        help="File with gold predictions (either English or German dev set of StereoSet)",
    )
    parser.add_argument(
        "--output_file",
        action="store",
    ...
    print(f" - gold_file: {args.gold_file}")
    ...
    args.output_file = f"./results/stereoset/{args.output_file}"
    # Either loop through a number of prediciton files or evaluate a single prediciton file
    if args.predictions_dir is not None:
        args.predictions_dir = f"./results/stereoset/{args.predictions_dir}"
        predictions_dir = args.predictions_dir
        ...
        for prediction_file in glob.glob(predictions_dir + "*.json"):
            ...
            parse_file(
                f"./data/stereoset/{args.gold_file}", prediction_file
            )
    else:
        parse_file(
            f"./data/stereoset/{args.gold_file}", args.predictions_file
        )