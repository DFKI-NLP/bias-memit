"""
A script to compute causal traces for a set incomplete input prompts
about factual associations.
Adaptations for the tracing of stereotypical associations are marked accordingly
...
def main():
    ...
    # Model selection changed to LMs supported for MEMIT de-biasing
    aa(
        "--model_name",
        default="gpt2-xl",
        choices=[
...
def calculate_hidden_flow(
    ...
    print(decode_tokens(mt.tokenizer, [answer_t])[0])
...
def decode_tokens(tokenizer, token_array):
    ...
    print([tokenizer.decode([t]) for t in token_array])
...
def get_embedding_cov(mt):
    ...
    def get_ds():
        ...
        raw_ds = load_dataset(
            ds_name,
            dict(wikitext="wikitext-103-raw-v1", wikipedia="20200501.en")[ds_name], # Using a German subset of Wikipedia for German LMs did not improve preformance
        )
        try:
